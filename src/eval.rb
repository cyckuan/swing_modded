...
def run_ROUGE(peer_root, model_root)
    ...
    File.open(rouge_in_file, 'w') do |f|
        ...
        Dir.glob(peer_root+'/*').sort.each do |peer|
            ...
            # model_files = Dir.glob(model_root+'/'+id.chomp('-A')+'*').map {|l| File.basename(l)}
    ...
    rouge_dir = '../lib/RELEASE-1.5.5'
    `#{rouge_dir}/ROUGE-1.5.5.pl -l 130 -n 4 -w 1.2 -m -2 4 -u -c 95 -r 1000 -f A -p 0.5 -t 0 -a -e /data/swing/SWING/lib/RELEASE-1.5.5/data -d #{rouge_in_file} > #{rouge_out_file}` #change to -l 100
